...
pip3 install ticklish_ui
   ...
   Check that the install worked by running the following code.

   #+begin_src python
from ticklish_ui import *

Application('MyApp').mainloop()
   #+end_src

   You should get an empty window with 'MyApp' in the titlebar.
...
>>> help(ticklish_ui.events.EventStream)
    ...
    demonstating how to use the various widgets.
