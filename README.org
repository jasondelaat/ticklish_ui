...
#ok_button.bind('<ButtonRelease-1>', lambda event: do_something(event))
  ...
  calls the destory() method on the toplevel window that contains
  it. That's why it can quit the program without an explicity event
  binding. Similarly there are RadioGroup and CheckGroup widgets which
  allow you to lay out whole sets of the corresponding buttons easily.
